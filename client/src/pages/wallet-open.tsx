...
import { ExternalLink, Smartphone, CheckCircle2, AlertCircle, Download, Copy, Check } from "lucide-react";
...
export default function WalletOpenPage() {
  ...
  const handleWallet = (w: WalletDef) => {
    ...
    // TokenPocket deep-link is unreliable — show manual instructions instead
    if (w.id === "tokenpocket") {
      setTpManual(true);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-5 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm space-y-5 relative z-10 slide-in">
        ...
        <div className="premium-card rounded-2xl p-5 space-y-5">
                ...
                <div key={w.id} className="space-y-1">
                  <button
                    ...
                    className={`relative w-full flex items-center gap-2.5 p-3 rounded-xl border transition-all text-left disabled:opacity-60 ${
                      ...
                      isTP && tpManual   ? `${w.cardCls} ring-1 ring-inset ring-emerald-500/20` :
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isTP && tpManual  && <p className="text-[9px] text-cyan-400">Open manually ↓</p>}
                    </div>
                  </button>
                  {isFailed && !isTP && (
                    <div className="flex items-center justify-between px-2">
          ...
          {/* TokenPocket manual instructions panel */}
          {tpManual && (
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-3.5 space-y-3">
              <div className="flex items-start gap-2">
                <img src="/tokenpocket-logo.svg" alt="TP" className="w-5 h-5 mt-0.5 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-emerald-300">Open in TokenPocket DApp Browser</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5 leading-relaxed">
                    Deep-link not supported — copy the link below and paste it in TokenPocket's browser.
                  </p>
                </div>
              </div>
              <ol className="space-y-1 pl-1">
                {["Open TokenPocket app", "Tap the Discover tab (bottom bar)", "Tap the search/URL bar", "Paste the link and go"].map((step, i) => (
                  <li key={i} className="flex items-center gap-2 text-[10px] text-muted-foreground">
                    <span className="w-4 h-4 rounded-full bg-emerald-500/20 text-emerald-400 text-[9px] font-bold flex items-center justify-center shrink-0">{i + 1}</span>
                    {step}
                  </li>
                ))}
              </ol>
              <button
                onClick={copyLink}
                className="w-full flex items-center justify-center gap-2 py-2 rounded-lg border border-emerald-500/40 bg-emerald-500/10 hover:bg-emerald-500/20 transition-all text-xs font-medium text-emerald-300"
                data-testid="button-copy-tp-link"
              >
                {copied
