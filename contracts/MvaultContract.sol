...
contract MvaultContract is Ownable, ReentrancyGuard {
    ...
    function _doActivate(address user, uint256 pkgPrice, uint256 incomeCap) internal {
        ...
        uint256 minted;
        {
            ...
            minted = mvaultToken.balanceOf(address(this)) - before;
        }
    ...
    function _routeSellNet(User storage u, address seller, uint256 mvtAmt, uint256 net, uint256 btc) internal {
        ...
        uint256 toRebirth;
        if (u.incomeLimit > 0) {
            toIncome  = net > u.incomeLimit ? u.incomeLimit : net;
            toRebirth = net - toIncome;
        ...
        emit MvtSold(seller, mvtAmt, net, btc, toIncome, toRebirth);
        _recordTx(seller, TX_SELL_MVT, net, 0, address(0));
    }
    ...
    /**
     * @notice Sell MVT from any pool (0=admin 1=community 2=reserve) → USDT direct to wallet.
     *         poolType: 0 = adminPool, 1 = communityPool, 2 = reservePool
     */
    function poolCashOut(uint8 poolType, uint256 amount, address recipient) external onlyOwner nonReentrant {
        if (amount == 0) revert ZeroAmount();
        if (recipient == address(0)) revert ZeroAddress();
        if (poolType == 0) {
            if (adminPool < amount) revert ExceedsPool();
    ...
    function setBtcPoolRate(uint8 _rate) external {
        if (!users[msg.sender].isRegistered || _rate < 5 || _rate > 80) revert InvalidParams();
